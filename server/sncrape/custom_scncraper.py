...
class Tweet(base.Item):
	...
	outlinksss = base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks), 'outlinks')
	tcooutlinksss = base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks), 'tcooutlinks')
...
class TwitterOldDesignScraper(base.Scraper):
...
class TwitterAPIScraper(base.Scraper):
			...
			for entry in entries:
				if entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-'):
					if 'tweet' in entry['content']['item']['content']:
						...
						raise base.ScraperException(f'Unable to handle entry {entry["entryId"]!r}')
...
class TwitterSearchScraper(TwitterAPIScraper):
	...
	def get_items(self,search_mode="image"):
		params = {
			...
			'q': self._query,
			#origin 'tweet_search_mode': 'live',
			'tweet_search_mode': search_mode,
			'count': '100',
